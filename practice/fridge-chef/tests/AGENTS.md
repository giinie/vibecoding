...
## Purpose
Pytest test suite with 66 tests covering all service modules. Tests use extensive mocking to avoid real API calls and in-memory SQLite for database tests.
...
| `test_auth.py` | 14 tests: AuthService registration, login, password hashing, profile/preference CRUD, account deletion |
...
| `test_sharing.py` | 10 tests: SharingService share link generation, QR codes, text formatting, enable/disable sharing |
...
- DB-dependent tests create in-memory SQLite with fresh schema per test
- Tests follow Arrange-Act-Assert pattern

### Testing Requirements
- All 66 tests must pass before committing
...
- Fixtures: in-memory DB session, sample user/recipe data
