// Local development server for PDF Summary App
...
async function handleSummarizeAPI(req, res) {
  let body = '';
  ...
  req.on('end', async () => {
    try {
      ...
      if (!text || text.trim() === '') {
        ...
        res.end(JSON.stringify({ error: '요약할 텍스트를 입력해주세요.' }));
        return;
      }
      ...
      if (text.length > MAX_TEXT_SIZE) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: '텍스트가 너무 깁니다. 500KB 이하의 텍스트만 처리할 수 있습니다.' }));
        return;
      }
...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/api/summarize' && req.method === 'POST') {
    handleSummarizeAPI(req, res);
    return;
  }

  // Static files
  let filePath = req.url === '/' ? '/index_pdf.html' : req.url;
...
server.listen(PORT, () => {
  console.log(`📄 PDF Summary App 서버가 시작되었습니다!`);
